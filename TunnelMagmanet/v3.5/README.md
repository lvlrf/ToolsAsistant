...
پس از اجرای generator، یک Dashboard تحت وب برای مدیریت راحت‌تر:

```bash
python3 generate-dashboard.py
```

سپس `dashboard-standalone.html` را باز کنید.

### ویژگی‌های Dashboard:

✅ **مشاهده همه کانفیگ‌ها** با جزئیات کامل  
✅ **فیلتر و جستجو** بر اساس سرور، transport، profile  
✅ **باز کردن Web Panel** با یک کلیک  
✅ **کپی دستورات** برای:
- وضعیت سرویس
- ری‌استارت سرویس
- مشاهده لاگ
...
**transports options:**
- `"all"`: همه transport ها (string)
- `["all"]`: همه transport ها (list)
- `["tcp", "ws", ...]`: لیست خاص
