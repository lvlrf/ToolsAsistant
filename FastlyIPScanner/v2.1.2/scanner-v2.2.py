            ...
            with open(json_file, 'w') as f:
                json.dump({
                    'metadata': {
                        ...
                        'status': 'interrupted',
                        'total': len(successful_ips)
                    },
                    'results': successful_ips
                }, f, indent=2)
            
            print(f"[OK] Local save: {txt_file}")
    except Exception as e:
    ...
    try:
        ...
        for cdn_name, ranges in CDN_RANGES.items():
            for cidr in ranges:
                if ip in ipaddress.ip_network(cidr):
                    return cdn_name
        # If using custom list, return 'custom' instead of 'unknown'
        if settings.get('using_custom_list', False):
            return 'custom'
        return 'unknown'
    except:
        if settings.get('using_custom_list', False):
...
def build_dns_query_txt(domain):
    """Build a DNS TXT query packet"""
    transaction_id = random.randint(0, 65535)
...
def build_dns_query_edns(domain, bufsize=1232):
    """Build a DNS query with EDNS0 OPT record"""
    transaction_id = random.randint(0, 65535)
    flags = 0x0100
    # Questions: 1, Answers: 0, Authority: 0, Additional: 1 (for OPT)
    header = struct.pack('>HHHHHH', transaction_id, flags, 1, 0, 0, 1)
    ...
    # Name: 0 (root)
    # Type: 41 (OPT)
    # UDP payload size: bufsize
    # Extended RCODE and flags: 0
    # RDLENGTH: 0
    opt = struct.pack('>BHHIH', 0, 41, bufsize, 0, 0)
    
    return header + query + opt

def parse_dns_response_detailed(data):
    """Parse DNS response with detailed info"""
    try:
        if len(data) < 12:
            return False, "Too short", {}
        ...
        edns_bufsize = 0
        if arcount > 0:
            # Simple check for OPT record (type 41)
            if b'\x00\x00\x29' in data:  # 0x0029 = 41 (OPT type)
                try:
                    opt_idx = data.index(b'\x00\x00\x29')
                    if opt_idx + 4 < len(data):
                        edns_bufsize = struct.unpack('>H', data[opt_idx+3:opt_idx+5])[0]
                except:
                    pass
        ...
        if rcode == 0:
            return True, "NOERROR", info
        elif rcode == 3:
            return True, "NXDOMAIN", info
        else:
            return False, f"DNS error: {rcode}", info
            
    except Exception as e:
        return False, str(e), {}

def test_dnstt(ip, port=53, timeout=5):
    """
    Layer 7: DNSTT Compatibility Test
    Tests if DNS server is suitable for DNS tunneling (dnstt)
    
    For DNSTT, DNS server is just a RELAY - content is encrypted
    So we only care about:
    1. DNS responds (any response is OK)
    2. TXT record support (dnstt uses TXT)
    3. EDNS0 support (for packet size)
    4. Stability (consistent responses)
    
    Hijack detection is NOT relevant - DNS is just forwarding encrypted data
    """
    results = {
        'success': False,
        'responds': False,
    ...
    # Test 1: Basic DNS Response (any response is OK)
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.settimeout(timeout)
        ...
        if data and len(data) >= 12:
            ...
            results['details'].append(f"✓ DNS Response: OK ({rtt:.0f}ms)")
        else:
            results['details'].append("❌ DNS Response: Empty/Invalid")
    except socket.timeout:
        results['details'].append("❌ DNS Response: Timeout")
        return results  # No point continuing if basic query fails
    except Exception as e:
        results['details'].append(f"❌ DNS Response: {str(e)[:30]}")
        return results
    
    # Test 2: TXT record support (important for dnstt)
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.settimeout(timeout)
        
        query = build_dns_query_txt('google.com')
        start = time.time()
        sock.sendto(query, (ip, port))
        data, _ = sock.recvfrom(4096)
        rtt = (time.time() - start) * 1000
        ...
        if data and len(data) >= 12:
            rtt_values.append(rtt)
            # Check if we got any answer (even error is info)
            flags = struct.unpack('>H', data[2:4])[0]
            qr = (flags >> 15) & 1
            ancount = struct.unpack('>H', data[6:8])[0]
            
            if qr == 1:  # Is a response
                if ancount > 0:
                    results['txt_support'] = True
                    results['tests_passed'] += 1
                    results['details'].append(f"✓ TXT Record: OK ({ancount} answers, {rtt:.0f}ms)")
                else:
                    # Response but no answers - still might work
                    results['txt_support'] = True
                    results['tests_passed'] += 1
                    results['details'].append(f"⚠ TXT Record: Response OK, no answers ({rtt:.0f}ms)")
        else:
            results['details'].append("❌ TXT Record: No response")
    except socket.timeout:
        results['details'].append("❌ TXT Record: Timeout")
    except Exception as e:
        results['details'].append(f"❌ TXT Record: {str(e)[:30]}")
    
    # Test 3: EDNS0 support
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.settimeout(timeout)
        
        query = build_dns_query_edns('google.com', 1232)
        start = time.time()
        sock.sendto(query, (ip, port))
        data, _ = sock.recvfrom(4096)
        rtt = (time.time() - start) * 1000
        ...
        if data and len(data) >= 12:
            ...
            # Try to find EDNS0 OPT record (type 41)
            edns_bufsize = 0
            if b'\x00\x00\x29' in data:
                try:
                    opt_idx = data.index(b'\x00\x00\x29')
                    if opt_idx + 4 < len(data):
                        edns_bufsize = struct.unpack('>H', data[opt_idx+3:opt_idx+5])[0]
            ...
            results['edns_bufsize'] = edns_bufsize
            
            if edns_bufsize >= 512:
                results['edns_support'] = True
                results['tests_passed'] += 1
                results['details'].append(f"✓ EDNS0: OK (bufsize={edns_bufsize})")
            elif edns_bufsize > 0:
                results['edns_support'] = True
                results['tests_passed'] += 1
                results['details'].append(f"⚠ EDNS0: Low bufsize ({edns_bufsize})")
            else:
                # Response but no EDNS - might still work with smaller packets
                results['details'].append(f"⚠ EDNS0: Not detected (may still work)")
        else:
            results['details'].append("❌ EDNS0: No response")
    except socket.timeout:
        results['details'].append("❌ EDNS0: Timeout")
    except Exception as e:
        results['details'].append(f"❌ EDNS0: {str(e)[:30]}")
    
    # Test 4: Stability (3 quick queries)
    stability_success = 0
    for i in range(3):
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    results['stability'] = stability_success
    stability_threshold = settings.get('dnstt_stability_threshold', 2)
    if stability_success >= stability_threshold:
        results['tests_passed'] += 1
        results['details'].append(f"✓ Stability: {stability_success}/3")
    else:
        results['details'].append(f"❌ Stability: {stability_success}/3 (need {stability_threshold})")
    
    # Calculate RTT stats
    if rtt_values:
        results['avg_rtt'] = round(sum(rtt_values) / len(rtt_values), 1)
        results['min_rtt'] = round(min(rtt_values), 1)
        results['max_rtt'] = round(max(rtt_values), 1)
        results['details'].append(f"📊 RTT: avg={results['avg_rtt']}ms min={results['min_rtt']}ms max={results['max_rtt']}ms")
    
    # Final verdict: Must respond + at least 2 other tests pass
    # More lenient: if it responds and is stable, it's probably usable
    if results['responds'] and results['tests_passed'] >= 2:
...
def scan_ip_port(ip, port, test_layers, parsed_config=None, xray_path=None):
    ...
    # Layer 7: DNSTT Compatibility Test (if selected)
    if 7 in test_layers:
        dnstt_result = test_dnstt(ip, 53, settings['timeout_dnstt'])
        result['tests']['dnstt'] = dnstt_result
        result['protocol'] = 'udp'
        result['dnstt_ready'] = dnstt_result.get('success', False)
        result['dnstt_tests_passed'] = dnstt_result.get('tests_passed', 0)
        result['avg_rtt'] = dnstt_result.get('avg_rtt', 0)
        result['edns_bufsize'] = dnstt_result.get('edns_bufsize', 0)
        
        if not dnstt_result.get('success'):
            result['success'] = False
            result['test_type'] = 'dnstt'
            result['dnstt_details'] = dnstt_result.get('details', [])
            with results_lock:
                tested_count += 1
                failed_ips.append(result)
            return result
    ...
    test_type_map = {1: 'port', 2: 'tls', 3: 'real', 4: 'download', 5: 'udp', 6: 'dns', 7: 'dnstt'}
    result['success'] = True
...
def perform_scan(ip_list, ports, test_layers, parsed_config=None, xray_path=None):
    ...
    # For DNS or DNSTT test, only use port 53
...
def save_results(results, duration):
    ...
    # Determine test type from first result
    test_type = results[0].get('test_type', 'unknown') if results else 'unknown'
    
    # TXT format - improved with test type info
    ...
    with open(txt_file, 'w') as f:
        ...
        f.write(f"# Channel: @drconnect\n\n")
        
        # Different format based on test type
        if test_type == 'dnstt':
            f.write("# Format: IP:PORT | CDN | Status | RTT | EDNS\n\n")
            for r in results:
                cdn = r.get('cdn', 'unknown')
                rtt = r.get('avg_rtt', 0)
                edns = r.get('edns_bufsize', 0)
                status = 'dnstt-ready' if r.get('dnstt_ready', False) else 'partial'
                f.write(f"{r['ip']}:{r['port']} | {cdn} | {status} | RTT:{rtt:.0f}ms | EDNS:{edns}\n")
        else:
            f.write("# Format: IP:PORT | CDN | Test | Latency\n\n")
            for r in results:
                cdn = r.get('cdn', 'unknown')
                latency = r.get('latency_ms', r.get('delay_ms', 0))
                f.write(f"{r['ip']}:{r['port']} | {cdn} | {test_type} | {latency:.0f}ms\n")
    ...
    # CSV format - improved with DNSTT columns
    csv_file = f'results/scan_{timestamp}.csv'
    with open(csv_file, 'w') as f:
        if test_type == 'dnstt':
            f.write('IP,Port,Protocol,CDN,Is_Official,Test_Type,DNSTT_Ready,Avg_RTT,EDNS_Bufsize,Tests_Passed\n')
            for r in results:
                f.write(f"{r['ip']},{r['port']},{r.get('protocol','udp')},{r.get('cdn','')},")
                f.write(f"{r.get('is_official','')},{r.get('test_type','')},")
                f.write(f"{r.get('dnstt_ready', False)},{r.get('avg_rtt','')},")
                f.write(f"{r.get('edns_bufsize','')},{r.get('dnstt_tests_passed','')}\n")
        else:
...
def menu_select_layers():
    ...
    print("[7] DNSTT Test         (DNS tunnel ready check)\n")
    print("Examples: 1,2 | 1,2,3 | 5 | 6 | 7 | 1,2,3,4\n")
    ...
    try:
        ...
        layers = [l for l in layers if l in [1, 2, 3, 4, 5, 6, 7]]
